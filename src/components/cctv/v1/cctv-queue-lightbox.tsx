 ...
 *
 * Koordinat zona memakai resolusi ACUAN dari pesan init wsai, yang TIDAK sama
 * dengan resolusi foto buktinya dan berubah-ubah: acuan pernah 1920×1080 lalu
 * jadi 640×360, sementara foto bukti pernah 960×540 lalu jadi 320×180. Karena
 * itu penskalaan bertumpu pada `aiRef` dan ukuran tampil, bukan pada ukuran asli
 * foto — begitu NVR mengubah salah satunya, gambar ini tetap benar tanpa perlu
 * diutak-atik.
 */
  ...
  return createPortal(
    ...
    >
      <div className="relative w-full max-w-[1240px] max-h-[94vh] flex flex-col bg-slate-900 border border-slate-700/70 rounded-3xl overflow-hidden shadow-2xl animate-pop-in">
        ...
        {/* Tinggi foto DIPATOK (`h-`, bukan `max-h-`) sebesar tinggi layar
            dikurangi header dan baris keterangan (±190px). Dua hal sekaligus:
              • tidak lagi terpotong — dulu `max-h-[62vh]` di dalam pembungkus
                `max-h-[94vh]` melebihi jatahnya pada layar pendek, lalu
                `overflow-hidden` memangkasnya diam-diam;
              • ikut MEMBESARKAN foto kecil. `max-h-` hanya membatasi, tak pernah
                meregangkan, sehingga bukti 320x180 tampil sebesar perangko.
            Gambar sumbernya memang cuma 320x180 (terukur pada 10 bukti terakhir),
            jadi hasil regangannya lunak — itu batas NVR-nya, bukan penskalaan di
            sini. `object-contain` + `max-w-full` menjaga rasionya tetap benar. */}
