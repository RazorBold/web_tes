...
function KartuLintasan({ hit }: { hit: VisitorHit }) {
...
export default function CctvVisitorHitsModal({ onClose }: Props) {
  ...
  const { data: hits, isFetching, error } = useCctvVisitorHits(true, 90, 40);
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3.5 border-b border-slate-800 flex-shrink-0">
          <div className="min-w-0">
            ...
            <p className="text-[12px] font-semibold text-slate-400">
              Lintasan 90 menit terakhir · menyegar tiap 10 detik
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4">
          {error ? (
            ...
            <p className="text-center text-[13px] font-semibold text-slate-400 py-10">
              Belum ada lintasan dalam 90 menit terakhir.
            </p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
              {hits.map((h) => (
                <KartuLintasan key={`${h.trackId}-${h.time}`} hit={h} />
              ))}
            </div>
          )}
        </div>
